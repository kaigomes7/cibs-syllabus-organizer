...
describe TamuCourse do
  describe "#create tamu_course" do
    before do
	  ...
	  @tamudepartment.save
	  @tamucourse = TamuCourse.new(course_num: 431, tamu_department_id: @tamudepartment.id, course_name: 'Software Engineering')
	  ...
	  @tamucourse_no_course_num = TamuCourse.new(tamu_department_id: @tamudepartment.id, course_name: 'Software Engineering')
...
describe Student do
  describe "#create student" do
    before do
	  ...
	  @student = Student.new(tamu_department_id: @tamudepartment.id, user_id: @user.id, tamu_major: 'CompSCi', tamu_college: 'Engineering', classification: 'Junior')
	  @student_invalid_department = Student.new(tamu_department_id: "-5", user_id: @user.id, tamu_major: 'CompSCi', tamu_college: 'Engineering', classification: 'Junior')
	  @student_no_department = Student.new(user_id: @user.id, tamu_major: 'CompSCi', tamu_college: 'Engineering', classification: 'Junior')
	  @student_invalid_user_id = Student.new(tamu_department_id: @tamudepartment.id, user_id: "-3", tamu_major: 'CompSCi', tamu_college: 'Engineering', classification: 'Junior')
	  @student_no_user_id = Student.new(tamu_department_id: @tamudepartment.id, tamu_major: 'CompSCi', tamu_college: 'Engineering', classification: 'Junior')
	  @student_no_major = Student.new(tamu_department_id: @tamudepartment.id, user_id: @user.id, tamu_college: 'Engineering', classification: 'Junior')
	  @student_no_college = Student.new(tamu_department_id: @tamudepartment.id, user_id: @user.id, tamu_major: 'CompSCi', classification: 'Junior')
	  @student_no_classification = Student.new(tamu_department_id: @tamudepartment.id, user_id: @user.id, tamu_major: 'CompSCi', tamu_college: 'Engineering')
...
describe ForeignCourse do
  describe "#create ForeignCourse" do
    before do
	  ...
	  @ForeignCourse_no_semester = ForeignCourse.new(foreign_course_name: 'Software Engineering', contact_hours: 3, foreign_course_num: 431, foreign_course_dept: 'CSCE', course_approval_status: false, tamu_department_id: @tamudepartment.id, university_id: @university.id)
	  @ForeignCourse_no_num = ForeignCourse.new(foreign_course_name: 'Software Engineering', contact_hours: 3, semester_approved: 'Fall 2020', foreign_course_dept: 'CSCE', course_approval_status: false, tamu_department_id: @tamudepartment.id, university_id: @university.id)
	  @ForeignCourse_no_dept = ForeignCourse.new(foreign_course_name: 'Software Engineering', contact_hours: 3, semester_approved: 'Fall 2020', foreign_course_num: 431, course_approval_status: false, tamu_department_id: @tamudepartment.id, university_id: @university.id)
	  @ForeignCourse_no_dept_id = ForeignCourse.new(foreign_course_name: 'Software Engineering', contact_hours: 3, semester_approved: 'Fall 2020', foreign_course_num: 431, foreign_course_dept: 'CSCE', course_approval_status: false, university_id: @university.id)
	  @ForeignCourse_no_uni_id = ForeignCourse.new(foreign_course_name: 'Software Engineering', contact_hours: 3, semester_approved: 'Fall 2020', foreign_course_num: 431, foreign_course_dept: 'CSCE', course_approval_status: false, tamu_department_id: @tamudepartment.id)
	  @ForeignCourse_null = ForeignCourse.new()
	end
	 
	it 'is valid with valid inputs' do
	  expect(@ForeignCourse).to be_valid
	end
	
	
	it 'is invalid: no name' do
	  expect(@ForeignCourse_no_name).not_to be_valid
	end
	
	it 'is invalid: no hours' do
	  expect(@ForeignCourse_no_hours).not_to be_valid
	end
	
	it 'is invalid: no semester' do
	  expect(@ForeignCourse_no_semester).not_to be_valid
	end
	
	it 'is invalid: no number' do
	  expect(@ForeignCourse_no_num).not_to be_valid
	end
	
	it 'is invalid: no department' do
	  expect(@ForeignCourse_no_dept).not_to be_valid
	end
	
	it 'is invalid: no department id' do
	  expect(@ForeignCourse_no_dept_id).not_to be_valid
	end
	
	it 'is invalid: no university id' do
	  expect(@ForeignCourse_no_uni_id).not_to be_valid
	end
	
	it 'is invalid: null' do
	  expect(@ForeignCourse_null).not_to be_valid
	end
