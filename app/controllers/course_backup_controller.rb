...
class CourseBackupController < ApplicationController
  def index
    redirect_to root_url, alert: 'You must be an admin to view that page, contact administrator if you believe this an error' unless admin? || current_user.role.zero?
  end
  ...
  def backup
    ...
    add_backup_departments(depts_file) if !depts_file.nil? && depts_file.content_type.in?(csv_file_types)
    add_backup_courses(courses_file) if !courses_file.nil? && courses_file.content_type.in?(csv_file_types)
    add_backup_majors(majors_file) if !majors_file.nil? && majors_file.content_type.in?(csv_file_types)
    add_backup_colleges(colleges_file) if !colleges_file.nil? && colleges_file.content_type.in?(csv_file_types)
  ...
  def add_backup_departments(depts_file)
    ...
    flash[:sucess] = "Added departments: #{new_departments.join(', ')}" if TamuDepartment.count > current_depts.length
  end
  ...
  def add_backup_courses(courses_file)
    ...
    new_courses.each do |course|
      ...
      dept = temp_course[0]
      new_courses_added << { tamu_department_id: current_depts_map[dept], course_num: temp_course[1], course_name: "#{dept} #{temp_course[1]}" } if current_depts_map.key?(dept)
    end
    TamuCourse.create!(new_courses_added)
    flash[:success] = "Added courses: #{new_courses_added.map { |nc| nc[:course_name] }.join(', ')}" if TamuCourse.count > current_courses.length
  end
